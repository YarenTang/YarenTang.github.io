...
我们都知道 Kotlin 集合基于 (Java 集合框架)[https://www.tutorialspoint.com/java/java_collections.htm]。理所应当，它的核心也是 `Iterator` 。  

#### Iterator

作为一个 Java 开发者，我们都知道 Iterator 主要的作用就是提供遍历的能力。但是，**Kotlin 将集合分成了两类： 「可变集合」 与  「不可变集合」**。 造成`Iterator` 层级核心变动如下：
...
![iterator](/images/2018/03/iterator.svg)
...
![kotlin collection hierarchy](/images/2018/03/collection.svg)
...
是不是很简单？ 我们尝试实现类似 `map`和 `filter` 结合的方法 `magicConvert`。
...
Kotlin 中因为 **操作的中间状态被快速地分配给了变量** ，则运行起来没有任何问题。

#### 延迟序列

Java 8 Stream 一个关键的点是：**它使用了惰性求值，即在需要的时候才会求值**。

而 **Kotlin 则相反**（除了 `sequences`，将在 `Lambda` 章节讲述)，采用及早求值。
...
以上代码，在 Kotlin 的版本中将执行5次 `map()` 和 `filter()` 操作，最后返回第一个值。而在 Java Stream 中集合操作只会各执行一次。 

在对性能有要求的场景下，我们需要 **使用 `asSequence（）` 方法将集合转为惰性序列**，以最小开销来实现业务。

#### 操作符

Java Stream 的中间操作与 Kotlin 几乎没有差别。唯一需要注意的是：
...
本篇文章简述了 Kotlin 集合的结构，揭露集合操作符的本质，初探扩展函数。

其次，通过与 Java 8 Stream 的比较，再次表现出 Kotlin 的优势。
