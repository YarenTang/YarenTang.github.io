...
这时你得面对一个难题：**只有一个洞，插耳机还是插电源！？**（在没有蓝牙耳机的前提下）  
...
在编程中，我们也会遇上类似的问题：
  ...
  2. 你想创建一个可以复用的类，该类可以与其他不相关的类或不可预见的类协同工作；  
  3. ...  
本文会通过 Adapter Pattern 来一步步解决上述问题。
...
class HeadsetAdapter extends PhoneJackConnector with Lightning {
  ...
  override def audioWithLightning: Unit = super.audioWithLightning
}
```
这时，你应该发现一个惊喜：在这个新的类里，我们可以对新老方法一起扩展——在 Java 中这是 「对象适配器」 和 「类适配器」 的一个差异。  
测试：
```scala
